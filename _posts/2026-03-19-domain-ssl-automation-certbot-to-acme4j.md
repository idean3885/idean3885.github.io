...
> **TL;DR**<br>
> - 앱 배포 플랫폼에 사용자 도메인 연결 기능이 추가되면서, 개별 도메인마다 SSL 인증서를 자동 발급해야 하는 과제가 생겼습니다.<br>
> - certbot으로 Let's Encrypt의 ACME 프로토콜을 먼저 학습한 뒤, HTTP-01 Challenge 기반으로 ACME4j 라이브러리를 Java BE에 통합했습니다.<br>
> - 상태 머신으로 인증서 생명주기를 관리하고, 배치 Job으로 발급·갱신·재시도·타임아웃을 처리하는 구조를 설계했습니다.<br>
> - 설계부터 QA까지 약 8주간 만난 실전 이슈(Rate Limit, 상태 교착, 이벤트 중복)와 해결 과정을 공유합니다.
{: .prompt-tip }

## 1. 배경 - 왜 인증서 자동화가 필요했는가

앱 배포 플랫폼을 운영하고 있습니다.
사용자가 서비스를 배포하면 `{서비스명}.platform.com` 도메인이 자동으로 할당되며,
플랫폼이 관리하는 와일드카드 인증서(`*.platform.com`)로 HTTPS가 제공됩니다.
...
사용자 도메인: my-service.example.com
  → *.platform.com 와일드카드 인증서로는 커버 불가
  → 개별 도메인에 대한 SSL 인증서가 필요
```

수동 발급은 선택지가 아니었습니다.
도메인 연결마다 관리자가 인증서를 발급하면 운영 비용을 감당할 수 없습니다.
...
2. **사용자 도메인 인증서는 HTTP-01이 적합하다고 생각했습니다**.
...
토큰의 일회성 특성을 고려하여
최종적으로 인메모리 HashMap이 적절하다고 생각하여 변경했습니다.
...
이를 동기 API로 처리하면 타임아웃 위험이 있다고 생각하여,
