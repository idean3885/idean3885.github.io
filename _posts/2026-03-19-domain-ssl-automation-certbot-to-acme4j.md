...
2. **사용자 도메인 인증서는 HTTP-01이 적합**합니다.
...
public class Certificate {
    private IssuanceStatusKind issuanceStatus;
    private ServiceRegistrationStatusKind serviceRegistrationStatus;
    private DomainServiceLinkStatusKind domainServiceLinkStatus;
    private int issuanceAttemptCount;
    private ZonedDateTime jobRunExpectAt;
    ...
    public void issueStart(Duration jobExpectDuration) {
        ...
        this.jobRunExpectAt = ZonedDateTime.now()
            .plus(jobExpectDuration);  // 타임아웃 감시용
    }

    // 발급 처리: PROCESSING → SUCCESS / FAILURE
    // 도메인 내부에서 issuerPort를 호출하는 Rich Domain 패턴
    public IssueProcessResponse issueProcess(
            IssueProcessRequest messageRequest) {
        var issuerPort = messageRequest.issuerPort();
        var renewalExpectDuration =
            messageRequest.renewalExpectDuration();
        try {
            var issuanceData = issuerPort.issue(this);
            ...
            this.jobRunExpectAt = this.expiredAt
                .minus(renewalExpectDuration);  // 갱신 예정
...
```java
// CertificateVerifyUserController
// — 외부(Let's Encrypt)에서 접근하는 엔드포인트
@GetMapping(
    "/.well-known/acme-challenge/{tokenName}")
public String getToken(
        @PathVariable String tokenName) {
    return certificateVerifyUserUseCase
        .getValid(tokenName);
}
```
...
처음에는 임베디드 Redis를 적용했지만,
토큰의 일회성 특성을 고려하여
최종적으로 인메모리 HashMap으로 변경했습니다.
...
public void verifyServiceLink(CertificateDomain domain) {
    ...
    String response =
        verifierFeign.getDomainVerifyData(domain);
...
public void issueAll() {
    List<Certificate> targets =
        repository.findAllIssueTargetLimit(
            ACCOUNT_SPEC.getMaxCharge()
            // Let's Encrypt Rate Limit 준수
        );
