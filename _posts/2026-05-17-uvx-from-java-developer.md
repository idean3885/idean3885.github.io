...
description: >-
  ...
  pyproject.toml 의 진입점, GitHub Actions 자동화, requires-python 무시 문제까지 한 번에 다룬다.
---

> **TL;DR**<br>
> uvx 는 PyPI 패키지를 격리 환경에서 실행하는 명령(=`uv tool run`). 자바의 jbang 자리에 가깝다.<br>
> 배포는 `pyproject.toml` 의 `[project.scripts]` 와 `uv build`/`uv publish`, GitHub Actions 의 validate→publish 2단계로 자동화한다.<br>
> 함정 하나: uvx 는 `requires-python` 을 의도적으로 무시한다. 사용자 환경 Python 이 낮으면 `setup.sh` 에서 `--python` 플래그를 자동 부여하는 것으로 막는다.
{: .prompt-tip }

## 1. 왜 uvx 인가

[claude-slack-to-notion](https://github.com/idean3885/claude-slack-to-notion) MCP 플러그인을 배포하면서 uvx 를 처음 접했다. MCP 공식 문서가 Python 서버 실행 방법으로 uvx 를 "recommended" 로 명시하고, Anthropic 공식 MCP 서버인 `mcp-server-git` 도 uvx 기반으로 배포된다. 사실상 Python MCP 플러그인의 표준 경로였다.

자바 개발자라서 처음엔 낯설었다. 대응표를 만들어보니 이해가 빨라졌다.
...
명령어만 다르고 하는 일은 거의 같다.

### uvx 의 정의

uvx 는 `uv tool run` 의 별칭. PyPI 패키지를 격리된 가상환경에서 실행하는 명령이다. Node 의 `npx` 와 같은 자리.
...
임시 실행 파일은 `~/.cache/uv/`, 영구 설치는 `~/.local/share/uv/tools/` 에 저장. MCP 서버처럼 매 세션마다 실행되는 경우에도 캐시 재사용으로 체감 성능 차이는 없다.
...
uvx 로 실행 가능한 패키지를 만들려면 두 가지가 필요하다. PyPI 에 배포된 패키지, 그리고 진입점이 정의된 `pyproject.toml`.
...
`[project.scripts]` 가 핵심. `uvx slack-to-notion-mcp` 를 실행했을 때 어떤 Python 함수가 호출될지 여기서 정의한다. 자바의 `MANIFEST.MF` `Main-Class` 자리. 형식은 `커맨드명 = "모듈.경로:함수명"`.
...
wheel 의 핵심은 **사용자 환경에서 빌드를 제거**하는 것. PyPI 자체는 빌드하지 않고 GitHub Actions 같은 곳에서 빌드한 결과물을 저장하는 저장소다 (Maven Central, npm registry 와 같은 자리).

### Git 직접 실행 대안 검토

uvx 는 PyPI 없이도 실행할 수 있다.

```bash
uvx --from git+https://github.com/user/repo slack-to-notion-mcp
```

PyPI 배포가 빠지니 매력적으로 보였다. 검토 후 기각:
...
대안을 검토해봐야 "왜 PyPI 를 쓰는가" 에 답이 명확해진다.

## 3. MCP 플러그인 연결: `.mcp.json`

PyPI 에 배포된 패키지는 `.mcp.json` 한 블록으로 연결된다.
...
처음에는 수동 `uv publish`. 태그 `v0.2.0` 인데 `pyproject.toml` 이 `0.1.9` 로 올라간 버전 불일치 사고 한 번을 겪고 자동화로 갔다.
...
핵심 설계는 `validate` → `publish` 2단계 분리. 태그 버전과 `pyproject.toml` 버전이 다르면 배포 전에 실패한다.
...
함정 하나: `GITHUB_TOKEN` 대신 `AUTO_TAG_PAT` 을 써야 한다. `GITHUB_TOKEN` 으로 생성한 태그는 다른 워크플로우를 트리거하지 않는다 (GitHub 의 무한 루프 방지 정책).
...
v0.2.0 배포 후 "Python 3.9 쓰는 사람한테는 왜 안 되지?" 라는 질문을 받았다. `pyproject.toml` 에 `requires-python = ">=3.10"` 을 기재했으니 3.9 사용자는 설치 단계에서 막힐 거라고 생각했다. **틀렸다.**

설치는 됐고, 실행도 됐고, 동작이 이상했다.

### uv 공식 문서

> "will ignore non-global Python version requests like .python-version files and the requires-python value"

`requires-python` 무시. `.python-version` 무시. 버그가 아니라 의도된 설계. GitHub 이슈 #8206, #14958 에서 이미 논의된 내용이다.

자바 관점에서 보면 이게 얼마나 낯선지 설명이 된다. Maven 에서 `<java.version>11</java.version>` 을 `pom.xml` 에 두면 JDK 8 로 빌드하려는 순간 컴파일 에러가 난다. uvx 는 그 체크 자체를 안 한다.
...
`uv run` 은 프로젝트 맥락이 있고, `uvx` 는 도구 격리 실행에 집중한다. 자바로 비유하면 `./gradlew run` 과 `jbang script.java` 의 차이.
...
시스템 Python 이 3.10 이상이면 그대로, 미만이면 `--python 3.10` 을 자동으로 붙인다. `--python 3.10` 지정 시 uv 가 자동으로 Python 3.10 을 찾거나 다운로드한다.

`requires-python` 은 지우지 않는다. `pip install` 사용자에게는 작동하고 PyPI 메타데이터 표시에도 쓰인다. uvx 에서만 무시될 뿐.

## 회고: 생태계가 다르면 안전장치도 다르다

Java 에서는 Maven/Gradle 이 JDK 버전을 강제한다. 버전 불일치는 빌드 도구가 막아준다. 그래서 `pom.xml` 에 버전을 쓰면 끝이라고 생각했다. Python uvx 에서는 그렇지 않다. 도구 실행 맥락에서는 설계 의도상 프로젝트 메타데이터를 보지 않는다.

알고 나면 간단한 이유가 있고 모르면 "왜 안 되지?" 에서 한참 헤맨다. 새 생태계로 옮길 때 가장 비싼 비용은 그 차이를 발견하는 시간이다.
...
동작하는 코드에 만족하지 않고 왜 그 방식인지 파고들면 새 생태계의 골격이 빠르게 보인다.
