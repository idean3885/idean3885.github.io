...
> (1) 쓰기 경합은 스키마 분리 + `@Transactional(readOnly)` 라우팅으로 해결. (2) 배치 진화는 내장 스케줄러 → CronJob → 이벤트 → 분산 4단계 청사진 + 전환 시그널. (3) 적용 패턴에 이름(HWM, Tumbling Window, Catch-up, Idempotency). (4) 합리적 UPSERT를 DELETE+INSERT로 통일(YAGNI).<br>
