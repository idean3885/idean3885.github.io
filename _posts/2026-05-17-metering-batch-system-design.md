---
title: "미터링 배치 시스템 설계 응축: 쓰기 경합·청사진·패턴 명명·저장 전략 통일까지"
...
> **TL;DR**<br>
> 5분 수집 / 10분 집계 / 일간 집계 미터링 파이프라인을 만들며 네 단계 의사결정을 거쳤다.<br>
...
운영 중인 클라우드 GPU 서비스의 과금 근거가 되는 사용량 데이터를 빠짐없이 수집·집계해야 했다. 설계부터 구현까지 본인 담당.
...
서비스 수가 늘면 배치 실행 시간이 길어지고 겹침 확률이 높아진다. 2년 예측 (분기당 50 서비스 증가) 으로 누적 약 2억 건. 단일 데이터소스로는 한계.
...
CQRS 라고 하면 이벤트 소싱이나 별도 읽기 저장소를 떠올리기 쉽지만, **스키마 분리 + 트랜잭션 라우팅만으로도 쓰기/읽기 독립 이점을 얻을 수 있었다**. 시스템 규모에 맞는 수준 선택이 더 중요하다.

## 2. 배치 진화 청사진: Stage 0 → 3 + 전환 시그널

핵심 원칙은 **전환 시그널이 나타날 때까지 현재 단계 유지**. 미리 과도하게 설계하면 복잡성만 늘어난다.

### Stage 0: 내장 스케줄러 + 상시 서버 (현재)

JobRunr `BackgroundJobServer` 가 주기적으로 Job 폴링. 메서드명이 곧 Job 이름이라 이름을 바꾸면 새 Job 으로 인식되어 이력이 단절된다. 이 제약은 **Job 이력 일관성이 실패 복구의 기반**이라 의도적.
...
상시 점유 → 실행 시에만 리소스. GraalVM Native Image 로 기동 ~0.5초, 메모리 128MB 로 줄이면 CronJob 의 기동 오버헤드 문제가 크게 풀린다.
...
소스 종류가 늘면 시간 기반 집계 (10:10) 시점에 일부 소스 데이터가 아직 없을 수 있다. 이벤트 기반이면 **모든 소스 수집 완료 후** 집계 시작.

대가: Exactly-once 처리. Stage 0 부터 유지한 멱등성 (UPSERT) 이 빛을 발한다.
...
배치를 만들면서 "이게 안전하겠다" 는 직관으로 결정한 것들이 사실 공식 이름이 있는 패턴이었다.
...
같은 UseCase 메서드의 오버로드로 표현한다. 비즈니스 능력은 "집계"로 동일하고 트리거 방식만 다르기 때문.
...
1. **의사소통이 정확해진다**: "HWM 기반 Incremental Load" 한 문장으로 줄어든다
2. **선택지가 보인다**: "Incremental Load" 안에 HWM 외에도 Snapshot Diff, CDC 가 있다
3. **검색이 된다**: "Catch-up pattern batch" 로 정확한 사례·베스트 프랙티스를 찾을 수 있다

## 4. 저장 전략 통일: UPSERT → DELETE + INSERT (YAGNI)

### UPSERT 선택의 합리성

미터링 배치 설계 초기 `INSERT ... ON DUPLICATE KEY UPDATE` 선택 근거는 명확했다.
...
**합리적인 설계였지만 우리 규모에서는 오버 엔지니어링이었다.**

> AI 에게 아무리 딥리서치를 시키고 다른 사례를 봐도 이런 결론은 나오지 않았다. "~2,000 Pod 상한의 B2B2C GPU 서비스에서 5인이 유지보수합니다" 라는 맥락은 어떤 리서치에도 다뤄지지 않는다. 이 판단은 서비스 맥락을 이해하는 팀원에게서 나왔다.
{: .prompt-info }
...
실측 성능 차이는 현재 규모에서 밀리초 단위. 소규모에서는 체감되지 않는다.

**성능이 비슷하다면 더 단순한 쪽이 이긴다.** 5인 팀에서 JDBC 하드코딩 SQL 관리 비용은 성능 이점을 상회한다.
...
일반론으로 옳은 결정이 특정 맥락에서는 오버 엔지니어링이 된다. 돌이켜보면 YAGNI 위배였다. 현재 필요하지 않은 동시성 보호를 미리 설계한 것이 비용이 되었다. 그 경계를 판단하는 것은 코드가 아니라 사람이다.
...
배치 시스템 설계의 핵심 역량은 **적정 설계 선택**이라고 생각한다. 다음 단계로의 전환 시점을 판단하는 능력이 정확한 청사진보다 중요하다.

이 시스템의 면접 빈출 주제 두 가지는 별도 글로 분리했다.
