...
| **Incremental Load** | 변경분만 탐지하여 적재 | 저비용, 빠름 | 변경 탐지 메커니즘 필요 |
...
> **Hard Delete 감지 필요 여부**와 **실시간 지연 요구**입니다.<br>
> Polling(주기적 쿼리)은 구현이 간단하지만
> 원본 DB에 지속적 부하를 주고 Hard Delete를 감지할 수 없습니다.<br>
> CDC는 인프라 구성이 복잡하지만 부하가 낮고
> INSERT/UPDATE/DELETE 모두 캡처 가능합니다.<br>
...
데이터를 고정 크기 단위로 읽기 → 가공 → 쓰기를 반복합니다.
Spring Batch의 기본 처리 모델이며, 메모리 사용량 예측과 실패 청크 재처리가 장점입니다.
`chunk-size`가 너무 작으면 오버헤드, 너무 크면 메모리와 락 문제가 생깁니다.
...
| 계층 | 전략 | 근거 |
|------|------|------|
| 원천 수집 계층 | ON DUPLICATE KEY UPDATE | 재수집 시 최신 값 갱신 |
| 구간 집계 (서비스) | UPSERT (PK 유지) | 일간 배치 조회 중 행 소실 방지 |
| 구간 집계 (Pod) | DELETE + INSERT (전체 교체) | 외부 참조 없음. 삭제-삽입 사이 짧은 공백이 생기지만, 이 계층은 외부에서 직접 조회하지 않으므로 허용 가능 |
...
반복 실패 레코드를 격리해 나머지 처리를 이어가는 패턴입니다.
도입 시 적재량 모니터링, 알림, 원인 분석 프로세스를 함께 설계해야 합니다.
...
핵심은 Retryable(네트워크 타임아웃, 5xx)과 Non-retryable(4xx, 데이터 오류)의 구분입니다. Non-retryable 재시도는 리소스 낭비로 끝납니다.
