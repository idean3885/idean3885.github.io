...
categories: [개발 기록, 클라우드 리소스 사용량 측정]
tags: [설계, 아키텍처]
description: >-
...
> **TL;DR** — 미터링 배치를 내장 스케줄러(JobRunr) + 상시 서버로 시작하고, 규모에 따라 CronJob 분리 → 이벤트 기반 → 분산 처리로 점진 확장하는 청사진을 설계했습니다.
...
5분마다 이 비용을 지불하는 것보다, 서버를 상시 띄워놓고 JobRunr의 BackgroundJobServer가 주기적으로 Job을 폴링하는 편이 합리적이었습니다.
...
| **해시 분할** | hash(workloadId) % N | 균등 분배 | 리밸런싱 복잡 |
...
배치 설계에서 가장 중요한 역량은 **현재 규모에 맞는 적정 설계를 선택하고, 다음 단계로의 전환 시점을 판단하는 능력**이라고 생각합니다.
